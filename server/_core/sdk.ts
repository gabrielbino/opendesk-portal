...
class SDKServer {
  ...
  async authenticateRequest(req: Request): Promise<User | AuthenticatedUser> {
    ...
    // Cron short-circuit
    if (sessionUserId.startsWith(CRON_OPEN_ID_PREFIX)) {
      const userInfo = await this.getUserInfoWithJwt(sessionCookie ?? "");
      if (!userInfo.taskUid) throw ForbiddenError("Cron session missing taskUid");
